...
export default function GoogleDayRoute({map,plan,visits,day,onSelect}:{map:google.maps.Map|null;plan:Trip;visits:any[];day:any;onSelect:(v:any)=>void}){
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(storage)||'{}');setPreferences(Object.fromEntries(Object.entries(saved.modes||{}).filter(([,v])=>typeof v==='string'&&v in modes)) as Record<string,Mode>);setAddresses(Object.fromEntries(Object.entries(saved.addresses||{}).filter(([,v])=>typeof v==='string'&&v.length<300)) as Record<string,string>)}catch{}},[]);
 const save=(nextModes:Record<string,Mode>,nextAddresses:Record<string,string>)=>{setPreferences(nextModes);setAddresses(nextAddresses);try{localStorage.setItem(storage,JSON.stringify({modes:nextModes,addresses:nextAddresses}))}catch{}};
 ...
 {missing&&<form className="route-address" onSubmit={e=>{e.preventDefault();const value=String(new FormData(e.currentTarget).get('address')||'').trim();if(value)save(preferences,{...addresses,[`${date}:${stop.id}`]:value})}}><label>Адрес не опубликован <input name="address" maxLength={299} placeholder="Укажите точный адрес и город" aria-label={`Адрес: ${stop.title}`} required/></label><button type="submit">Добавить в мой маршрут</button><small>Адрес сохранится только в этом браузере и будет передан Google для построения пути.</small></form>}
